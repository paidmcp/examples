...
const coingeckoIdSchema = z
  ...
  .regex(
    /^[a-z0-9-]+$/,
    "Use a lowercase CoinGecko ID like bitcoin, ethereum, tether",
  );
...
export const tools: Array<PaidTool<any, any>> = [
  definePaidTool({
    name: "get_price",
    description:
      "Get current USD price and 24h change for a token by CoinGecko ID.",
    priceUsdt: 0.001,
    inputSchema: z.object({
      id: coingeckoIdSchema.describe(
        "CoinGecko ID like bitcoin, ethereum, tether",
      ),
    }),
    handler: async ({ id }) => {
