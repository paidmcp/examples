...
const mcp = new Server({ name: "crypto-prices-mcp", version: "0.1.0" }, { capabilities: { tools: {} } });
...
mcp.setRequestHandler(CallToolRequestSchema, async (request) => ({
  ...
  content: [
    {
      type: "text",
      text: `Use paidmcp-client or HTTP POST /tools/${request.params.name} to handle x402 payment.`
    }
  ]
}));
...
app.listen(config.PORT, () => {
  console.log(`crypto-prices-mcp running on http://localhost:${config.PORT}`);
});
